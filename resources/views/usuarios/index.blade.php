                ...
                <div class="overflow-x-auto custom-scrollbar">
                    <table class="ui-table">
                        <thead>
                            <tr>
                                <th>Usuário</th>
                                <th class="text-center">Cargo Hierárquico</th>
                                    ...
                                    <td class="text-right">
                                        <div class="ui-row-actions justify-end">
                                            ...
                                            @if ($user->id !== auth()->id() && $user->role !== 'master')
                                                <form id="del-user-{{ $user->id }}" action="{{ route('usuarios.destroy', $user->id) }}" method="POST">
                                                    @csrf @method('DELETE')
                                                    <button type="button" onclick="confirmAction({ title: 'Revogar Usuário', message: 'Deseja descadastrar e revogar o acesso deste usuário?', formId: 'del-user-{{ $user->id }}', confirmText: 'Revogar', variant: 'danger' })" class="p-2 rounded-xl text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:text-red-400 dark:hover:bg-red-500/10 transition-colors" title="Revogar Usuário">
