                        ...
                        @foreach($classe->requisitos->sortBy('categoria') as $req)
                            <div class="group flex items-start gap-3 p-3 rounded-xl border border-slate-100 dark:border-slate-700 hover:border-slate-200 dark:hover:border-slate-600 transition-colors">
                                ...
                                <div class="flex gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" x-show="editandoId !== {{ $req->id }}">
                                    ...
                                    <form id="del-req-{{ $req->id }}" action="{{ route('classes.requisitos.destroy', [$classe, $req]) }}" method="POST">
                                        @csrf @method('DELETE')
                                        <button type="button" onclick="confirmAction({ title: 'Excluir Requisito', message: 'Excluir este requisito? Os registros de conclusão dos desbravadores também serão removidos.', formId: 'del-req-{{ $req->id }}', confirmText: 'Excluir', variant: 'danger' })" class="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors" title="Excluir">
