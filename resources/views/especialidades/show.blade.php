                    ...
                    @foreach($especialidade->requisitosOficiais as $req)
                        <li class="group flex items-start gap-3 p-3 rounded-xl border border-slate-100 dark:border-slate-700 hover:border-slate-200 dark:hover:border-slate-600 transition-colors">
                            ...
                            <div class="flex gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" x-show="editandoId !== {{ $req->id }}">
                                ...
                                <form id="del-esp-req-{{ $req->id }}" action="{{ route('especialidades.requisitos.destroy', [$especialidade, $req]) }}" method="POST">
                                    @csrf @method('DELETE')
                                    <button type="button" onclick="confirmAction({ title: 'Remover Requisito', message: 'Remover este requisito?', formId: 'del-esp-req-{{ $req->id }}', confirmText: 'Remover', variant: 'danger' })" class="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors" title="Remover">
